...
//             }
//             space->saveAvg = true;
//             // 14 -- потому что период такой. До этого было 144, но это при t = 1e-16
//             for (size_t gr = 0; gr < 14; ++gr) space->MDStep();
//             eV = space->avgVibEn / 14;
//             eR = space->avgRotEn / 14;
//             auto tmp = std::chrono::steady_clock::now();
...
int main(const int argc, char* argv[]) {
    ...
    std::ifstream fin(pars.getCfgInpFile());
    std::string tStr;
    std::vector<std::vector<double>> coord1;
    ...
    for (size_t i = 0; i < coord1.size(); ++i) {
        ...
        while (!space->MDStep() && step < MAXSTEPS) {
            ...
            //     std::cout << "step: " << step << "  |  " << static_cast<double>(std::chrono::duration_cast<std::chrono::milliseconds>(tmp - start).count()) / 1000 << " sec" << std::endl;
            // }
            if (step % 100 == 0) space->writeVTK(pars.getOutFile() + "_" + std::to_string(i));
            ++step;
        }
        space->writeVTK(pars.getOutFile() + "_" + std::to_string(i));
        double eV = 0, eR = 0, eT = 0;
        std::vector<double> vTmp(3, 0);
        if (!space->molsN2.empty()) {
            for (size_t in = 0; in < 3; ++in)
                vTmp[in] = ((space->molsN2[0].atom[0]->vel[in] + space->molsN2[0].atom[0]->vel2[in]) / 2 + (space->molsN2[0].atom[1]->vel[in] + space->molsN2[0].atom[1]->vel2[in]) / 2) / 2.0;
            for (size_t in = 0; in < 3; ++in) eT += vTmp[in] * vTmp[in];
            eT *= MASS_FOR_N / KB; 
        }
        space->saveAvg = true;
        ...
        if (step != MAXSTEPS) {
            outT.Out << "i = " << i << " ";
            saveInfo(outT, macro[0], macro[1], macro[2], macro[3], vTmp, eT, eV, eR, 1, timeT, step);
            outS.Out << "i = " << i << " ";
            for (auto x : startInf) outS.Out << x << " ";
                outS.Out << std::endl;
        }
        else { 
            std::cout << "BAD_CALC: " << macro[0] << " " << macro[1] << " " << macro[2] << " " << macro[3] << std::endl;
            for (auto x : startInf) std::cout << x << " ";
            std::cout << std::endl;
        }
        delete space;
