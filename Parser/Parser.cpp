...
Parser::Parser(const int argc, char* argv[]) {
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "--out") {
            if (++i == argc) {
                throw std::runtime_error(
                    "Expected value after key --out is empty");
            }
        ...
        if (key == "--cfginp") {
            if (++i == argc) {
                throw std::runtime_error(
                    "Expected value after key --cfginp is empty");
            }
...
Parser::Parser(const int argc, char* argv[], bool flag) {
    if (!flag) return;
    for (int i = 1; i < argc; i++) {
        auto key = std::string(argv[i]);
        if (key == "--cfg") {
        ...
        if (key == "--confs") {
            if (++i == argc) {
                throw std::runtime_error(
                    "Expected value after key --confs is empty");
            }
    ...
    if (N == 0) throw std::runtime_error("count confs == 0");
    if (cfgFile.empty()) throw std::runtime_error("CFG file not specified");
    if (outFile.empty()) throw std::runtime_error("OUT file not specified");
    if (cfgInpFile.empty()) throw std::runtime_error("CFGINP file not specified");
    isLoaded = true;
}
...
const int& Parser::getCountConfs() const {
    if (not isLoaded) throw std::runtime_error("CommLine not yet parsed");
    return N;
}